...
const Notif = (msg, type) => {
    const notif = (<box
        className={`auth-notif ${type}`}
    >
        <label label={msg} maxWidthChars={25} wrap={true} />
    </box>);

    const revealer = (<revealer halign={Gtk.Align.END} transitionType={Gtk.RevealerTransitionType.SLIDE_LEFT} transitionDuration={250} revealChild={false}
    >{notif}</revealer>);
...
function LoginBox() {
    ...
    >
        ...
        />
        <box
            className={bind(inputNeeded).as(n => `entry-box ${n ? "" : "hidden"}`)}
            vertical
        >
