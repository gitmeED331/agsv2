...
function WifiAP(ap) {
            ...
            onClick={(_, event) => {
                if (event.button === Gdk.BUTTON_PRIMARY) {
                    execAsync(`nmcli con down ${ap.ssid}`)
                        .then(() => { execAsync(`notify-send "WiFi" "Disconnected from ${ap.ssid}"`) },
                            (error) => {
    ...
    const APForget = (
        <button
            ...
            onClick={(_, event) => {
                if (event.button === Gdk.BUTTON_PRIMARY) {
                    execAsync(`nmcli connection delete ${ap.ssid}`)
                        .then(() => {
    ...
    const APItem = (
        <box vertical={true}>
            <centerbox
                ...
                startWidget={APEntry()}
                endWidget={
                    <stack className={"wifi connected controls"} visible={isActiveAP || isConnecting} halign={Gtk.Align.END}
                        visible_child_name={isConnecting ? "connectionSpinner" : "controls"}
                        homogeneous={false}>
                        <box name={"connectionSpinner"} halign={Gtk.Align.END}>
                            <Gtk.Spinner halign={Gtk.Align.END} valign={Gtk.Align.CENTER} />
                            <label
                                label={"Connecting..."}
